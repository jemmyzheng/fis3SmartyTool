...
module.exports = {
  ...
  * */
  ...
  isFis3Module: function isFis3Module(cwd,modulePath,module) {
    var readModule = this.getDefer();
    glob(path.join(cwd || '', modulePath || '',module,'fis-conf.js'), function (err, fileName) {
      if (err || !fileName.length) {
        readModule.reject(err);
      } else {
        readModule.resolve(fileName);
      }
    });
